...
| 2026-09-10 | Estado modelado **sem escritor** ainda carrega defeito: `pausada` não é gravada por código nenhum, e mesmo assim a pausa já vinha com a morte do token embutida — a renovação mora dentro da coleta, e a coleta só varria `ativa`. Quem escrevesse a suspensão de assinatura herdaria a perda sem saber | Varredura passou a incluir `pausada`, só para renovar (**ADR-011**). Consertar antes do escritor é mais barato que descobrir com cliente pausado |
| 2026-09-10 | Regra que a Edge Function decide sozinha não tem como ser provada: não há Deno no CI, e o que se garante sobre essas funções é o que o banco lhes entrega. A lista de quem a coleta varre era uma linha de TypeScript sem teste, e o erro dela — coletar quem pediu pausa, ou deixar um token morrer — não levanta exceção nenhuma | Lista virou `public.contas_da_varredura`, com asserção em Postgres de verdade. Regra que erra em silêncio desce para onde existe teste |
