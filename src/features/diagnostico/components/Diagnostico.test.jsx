...
describe('Diagnostico — os estados que não são a tela', () => {
  ...
  it('diagnóstico velho se declara velho, em vez de passar por atual', async () => {
    ...
    obterDiagnosticoMaisRecente.mockResolvedValueOnce({
      ...original,
      data: { ...original.data, geradoEm: '2026-08-24T04:40:00.000Z' },
    })
    ...
    expect(frescor).toHaveAttribute('data-estado', 'envelhecido')
    expect(frescor).toHaveTextContent('há 12 dias')
    // Não desmente o veredito: ele vale para a janela que comparou.
    expect(frescor).toHaveTextContent('continua valendo')
